...
function Vehicle(id, loc){
  this.id = id;
  this.loc = loc;
  this.vel = new JSVector(Math.random()*6, Math.random()*6);
  this.acc = new JSVector(Math.random()*3, Math.random()*4);
  this.angle = 0;
...
// Method to display
  ...
  this.antiseek = function(vehiclesList){
    for(var i = 1; i < vehiclesList.length; i++){
    var target = vehiclesList[i];
    if(target == this){
      continue;
    }
